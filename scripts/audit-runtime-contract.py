...
def render(*files):
    ...
    # The Docker Control override deliberately requires a socket GID which is
    # absent from the standard .env.example. Supply a harmless render-only GID
    # without weakening that runtime requirement or polluting standard mode.
    environment = os.environ.copy()
    if "compose.docker-control.yml" in files:
    ...
    try:
        ...
        raise RuntimeError(f"Compose render failed: {detail}") from error
