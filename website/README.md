...
Prerequisites: Node.js 22 and npm. Refresh the complete lock graph with `npm install --package-lock-only --ignore-scripts`, then run `npm ci` and `npm run dev`. Use `npm run check` for Astro/TypeScript validation, `npm run build` to generate `dist/`, and `npm run preview` to inspect that build. The Pages workflow performs the same lock synchronization before its clean install.
